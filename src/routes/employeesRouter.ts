...
import { DataTransformer, SQLStorage } from "../Storage";
import { EmployeeInfo, HiredEmployeeInfo, TABLENAMES } from "../types";

export const employeeRouter = Router();

employeeRouter.get("/employees/", async (req, res, next) => {
  let currentEmployees = await Employees.getAllEmployees(
    new SQLStorage(TABLENAMES.EMPLOYEES)
  );
...
employeeRouter.post("/employees/new", async (req, res, next) => {
  const employeebucket = new SQLStorage(TABLENAMES.EMPLOYEES);
